...
export const types = /^kiss$/i
export const nama = "Kiss"
export const kategori = "Fun"
export const bantuan = [
    getOptions()?.prefix + "hug [tag orangnya]"
]
...
export default async function Hello(socket: WASocket, {
    ...
    const feting = await axios.get("https://api.waifu.pics/sfw/kiss", {
    ...
    if (data.split(".").at(-1) == "gif") {
        ...
        try {
            ...
            await socket.sendMessage(room, {
                ...
                gifPlayback: true,
                caption: `${pesan.length == 1 ? "@" + pengirim.replace("@s.whatsapp.net", "") : pesan[0]} mencium ${pesan[ pesan.length == 1 ? 0: 1]}`,
    ...
    await socket.sendMessage(room, {
        ...
        caption: `${pesan.length == 1 ? "@" + pengirim.replace("@s.whatsapp.net", "") : pesan[0]} mencium ${pesan[1]}`,
        mentions: [...getMentions(pesan.join(" ") + " " + ("@" + pengirim.replace("@s.whatsapp.net", ""))) || ""]
    })
