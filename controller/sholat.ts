...
export default async function Sholat(
  ...
  try {
    ...
    if (!isKode) {
      const fetch = await axios.get(
        `https://api.banghasan.com/sholat/format/json/kota/nama/${pesan[0]}`
      );
      ...
      const fetch2 = await axios.get(
        `https://api.banghasan.com/sholat/format/json/jadwal/kota/${kota.id}/tanggal/` +
          `${date.getFullYear()}-${
            date.getMonth() + 1 < 10
              ? "0" + (date.getMonth() + 1)
              : date.getMonth() + 1
    ...
    const fetch = await axios.get(
      `https://api.banghasan.com/sholat/format/json/jadwal/kota/${pesan[0]}/tanggal/` +
        `${date.getFullYear()}-${
          date.getMonth() + 1 < 10
            ? "0" + (date.getMonth() + 1)
            : date.getMonth() + 1
    ...
    const fetch2 = await axios.get(
      `https://api.banghasan.com/sholat/format/json/kota/kode/${pesan[0]}`
    );
    const data = fetch.data;
    const data2 = fetch2.data;
    if (data.status != "ok") throw "Status is not Ok";
